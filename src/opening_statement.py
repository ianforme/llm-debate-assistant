...
from config import OPENAI_API_KEY, ORG_KEY, PROJECT_KEY, CONCURRENCY, RETRIES, JITTER

import os
from src.utils import rewrite_style, mk_notify
from style_examples.style_examples import opening_statement_style_example
from prompts.opening_statement_prompts import debate_outline_prompt, example_card_prompt, opening_statement_prompt, opening_statement_improver_prompt, opening_statement_evaluator_prompt
from output_structure import OpeningStatementOutline, Examples, OpeningStatement, OpeningStatementEvaluationFeedback
...
async def fetch_one_wrapped(
    ...
    for attempt in range(1, RETRIES + 1):
        try:
            ...
            await asyncio.sleep(backoff + random.random() * JITTER)
            backoff *= 2
...
async def opening_statement_improvement(opening_statement, debate_outline, topic, side):
    ...
    input_items = [{"content": f"opening statement to be evaluated: {opening_statement['opening_statement']}", "role": "user"}]
    counter = 1
    ...
    with trace(f"LLM as a judge-{datetime.datetime.now().strftime("%Y%m%d%H%M%S")}"):
        while True:
            ...
            input_items.append({"content": f"opening statement to be evaluated: {enhanced_opening_statement.final_output.opening_statement}", "role": "user"})
...
async def generate_opening_statement(topic, side, llm_as_judge=True, status_cb=None):
    ...
    if llm_as_judge:
        ...
        enhanced_opening_statement, _ = await opening_statement_improvement(opening_statement, debate_outline, topic, side)

        notify("=" * 10 + "Stage 5: 基于示例优化写作风格..." + "=" * 10 + "\n")
        final_opening_statement = rewrite_style(enhanced_opening_statement.opening_statement, opening_statement_style_example)

    else:
        notify("=" * 10 + "Stage 4: 基于示例优化写作风格..." + "=" * 10 + "\n")
        final_opening_statement = rewrite_style(opening_statement['opening_statement'], opening_statement_style_example)
