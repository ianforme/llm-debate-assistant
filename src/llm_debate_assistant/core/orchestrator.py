...
class DebateOrchestrator:
    ...
    async def generate_opening_statement(
        ...
        if llm_as_judge:
            ...
            (
                enhanced_opening_statement,
                _,
            ) = await self.assistant.opening_statement_improvement(
    ...
    def rebuttal_interrogation_practice(
        ...
        notify("开始质询环节...\n")
        speech_history = self.realtime_assistant.run(
            exchange_context, user_time_in_seconds, ai_start_first=False
        )
    ...
    def oregon_interrogation_practice(
        ...
        notify = mk_notify(status_cb)
        notify("初始化质询环节中...\n")
        exchange_context = oregon_interrogated_prompts(
            topic,
            assistant_side,
            assistant_baseline,
            assistant_statement,
