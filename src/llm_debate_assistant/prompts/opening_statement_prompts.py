...
- **重要**：你的反馈将被用于指导下一步的改进工作。如果选择 `redo_outline`、`redo_evidence` 或 `redo_draft`，你的反馈将直接传递给相应的生成节点，因此必须具体说明需要改进什么。

【重做决策指南】
当评估结果为"fail"时，你需要判断问题的根源并指定需要重做的步骤：

1. **redo_outline**: 如果问题出在论证框架本身
   - 关键词定义不当或不准确
   - 比较标准有严重缺陷或对己方不利
   - 论点结构混乱或论点之间缺乏逻辑连贯性
   - 论点不支持己方立场或存在根本性逻辑错误
   - **反馈示例**："比较标准'经济效益'过于狭隘，应改为'可持续发展'以涵盖社会和环境因素"

2. **redo_evidence**: 如果论证框架合理，但证据不足或不合适
   - 现有证据质量低、来源不可靠
   - 缺少关键统计数据或实证案例
   - 证据与论点关联度不强
   - 需要更有说服力的例子
   - **反馈示例**："论点二缺少具体数据支持，需要查找关于远程工作对生产力影响的量化研究"

3. **redo_draft**: 如果框架和证据都可以，但立论稿本身写作有问题
   - 表达不够清晰或口语化
   - 证据解释不充分
   - 逻辑过渡不流畅
   - 字数超标或结构不合理
   - **反馈示例**："证据引用过于学术化，需要用更通俗的语言解释'碳足迹'的概念"

4. **improve**: 如果只需要小幅改进（默认选项）
   - 局部表达优化
   - 微调证据使用方式
   - 改善措辞和语气
   - **反馈示例**："论点一和论点二之间的过渡生硬，建议加入承接句"
...
- 输出必须包含：
  ...
  3. `next_action`: （仅当 evaluation_result 为 "fail" 时需要）指定下一步行动，可选值为 "redo_outline"、"redo_evidence"、"redo_draft" 或 "improve"
